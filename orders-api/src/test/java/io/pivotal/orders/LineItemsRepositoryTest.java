...
public class LineItemsRepositoryTest {
    ...
    public void canCreateFindAndDeleteALineItem() {
        ...
        UUID orderId = ordersRepo.create(Order.newInstance().requestedBy("tony.stark@starkenterprises.com").orderedBy("nick.fury@shield.com").branch("AVE").supplier("Stark Enterprises").remarks("for Thanos").status("REQUESTED"));
        UUID lineItemId = lineItemsRepo.create(LineItem.newInstance().orderId(orderId).itemCode("BERTHA").itemDescription("Near Earth orbit Hulk buster suit of armor").quantity(1L).unitPrice(15000000.00).unitOfMeasure("PC"));
